...
def read_comment_args(request):
    ...
    return name, email, title, content, tags, url, reply_to_id, format, start_wrapper, end_wrapper, start_offset, end_offset

def validate_comment_args(name, email, title, content, tags, url):
    errors = {}
    if name != None : 
        if name == "" :
...
#    and comment.user == self.request.user
    ...
    # TODO check version still exist ...
        ...
        comment = Comment.objects.create(state=comment_state, text_version=text_version, user=user, name=name, email=email, title=title, content=content, content_html=content_html, tags = tags, url=url, start_wrapper = start_wrapper, end_wrapper = end_wrapper, start_offset = start_offset, end_offset = end_offset, reply_to=reply_to)
